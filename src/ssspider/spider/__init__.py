...
from spider.workflow import Workflow
from spider.exceptions import print_exception, WorkflowError
from spider.logging import setup_logger, logger
from spider.version import __version__
from spider.io import load_configfile
from spider.shell import shell
from spider.utils import update_config, available_cpu_count
from spider.common import Mode

def spider(spiderfile,
              listrules=False,
              ...
              cluster_status=None):
    """Run spider on a given spiderfile.

    This function provides access to the whole spider functionality. It is not thread-safe.

    Args:
        spiderfile (str):            the path to the spiderfile
        ...
        jobname (str):              naming scheme for cluster job scripts (default "spiderjob.{rulename}.{jobid}.sh")
        ...
        spidermakepath (str):        Deprecated parameter whose value is ignored. Do not use.
        ...
        print_compilation (bool):   print the compilation of the spiderfile (default False)
        ...
        keep_shadow (bool):         Do not delete the shadow directory on spider startup.
        ...
        mode (spider.common.Mode): Execution mode
        ...
        cluster_status (str):       Status command for cluster execution. If None, Spider will rely on flag files. Otherwise, it expects the command to return "success", "failure" or "running" when executing with a cluster jobid as single argument.
        log_handler (function):     redirect spider output to this custom log handler, a function that takes a log message dictionary (see below) as its only argument (default None). The log message dictionary for the log handler has to following entries:
    ...
    if not os.path.exists(spiderfile):
        logger.error("Error: Spiderfile \"{}\" not present.".format(spiderfile))
        return False
    spiderfile = os.path.abspath(spiderfile)
    ...
    try:
        ...
        if default_remote_provider is not None:
            try:
                rmt = importlib.import_module("spider.remote." +
        ...
        workflow = Workflow(spiderfile=spiderfile,
                        jobscript=jobscript,
        ...
        # ignore this exception and stop. It occurs if spider output is piped into less and less quits before reading the whole output.
        # in such a case, spider shall stop scheduling and quit with error 1
        success = False
...
def get_argument_parser(parser = None):
    ...
    if parser is None:
        parser = argparse.ArgumentParser(
            description="Spider is a Python based language and execution "
    ...
    parser.add_argument("--spiderfile", "-s",
                        metavar="FILE",
                        default="Spiderfile",
                        help="The workflow definition in a spiderfile.")
    # 禁用GUI
    # parser.add_argument(
    #     "--gui",
    #     nargs="?",
    ...
    #     "port e.g. 168.129.10.15:8000. By default Spider is only "
    #     "available in the local network (default port: 8000). To make "
    #     "Spider listen to all ip addresses add the special host address "
    #     "0.0.0.0 to the url (0.0.0.0:8000). This is important if Spider "
    ...
    parser.add_argument("--list", "-l",
                        action="store_true",
                        help="Show availiable rules in given Spiderfile.")
    parser.add_argument("--list-target-rules", "--lt",
                        action="store_true",
                        help="Show available target rules in given Spiderfile.")
    parser.add_argument("--directory", "-d",
                        metavar="DIR",
                        action="store",
                        help=("Specify working directory (relative paths in "
                              "the spiderfile will use this as their origin)."))
    ...
    parser.add_argument(
        ...
        "use on Unix systems: spider --dag | dot | display")
    parser.add_argument(
        "--force-use-threads",
    ...
    parser.add_argument(
        ...
        "Recommended use on Unix systems: spider --rulegraph | dot | display")
    ...
    parser.add_argument(
        ...
        "Note that it is best practice to have the Spiderfile, config files, and "
    ...
    parser.add_argument(
        ...
        help=("Touch output files (mark them up to date without really "
              ...
              "fool future invocations of spider. Fails if a file does "
              "not yet exist."))
    ...
    parser.add_argument(
        ...
        help=("Don't check for ambiguous rules and simply use the first if "
              ...
              "prioritize rules by their order in the spiderfile."))
    ...
    cluster_group.add_argument(
        ...
        metavar="CMD",
        help=
        ("Execute spider rules with the given submit command, "
         "e.g. qsub. Spider compiles jobs into scripts that are "
         ...
         "$ spider --cluster 'qsub -pe threaded {threads}'.")),
    cluster_group.add_argument(
        "--cluster-sync",
        metavar="CMD",
        help=
    ...
    cluster_group.add_argument(
        ...
        const="",
        metavar="ARGS",
        help="Execute spider on a cluster accessed via DRMAA, "
        "Spider compiles jobs into scripts that are "
        "submitted to the cluster with the given command, once all input "
    ...
    parser.add_argument(
        ...
        " in which case Spider will use the current invocation directory"
    ...
    parser.add_argument(
        ...
        ("A JSON or YAML file that defines the wildcards used in 'cluster'"
         "for specific rules, instead of having them specified in the Spiderfile. "
    ...
    parser.add_argument(
        ...
        "$ spider --cluster 'sbatch --dependency {dependencies}.\n"
    ...
    parser.add_argument(
        "--jobname", "--jn",
        default="spiderjob.{rulename}.{jobid}.sh",
        metavar="NAME",
        help="Provide a custom name for the jobscript that is submitted to the "
        "cluster (see --cluster). NAME is \"spiderjob.{rulename}.{jobid}.sh\" "
    ...
    parser.add_argument(
        ...
        "in combination with the --cluster flag. If provided, Spider will "
        ...
        "will be invoked with the job id. Spider expects it to return "
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        help=
        "Write stats about Spiderfile execution in JSON format to the given file.")
    ...
    parser.add_argument(
        ...
        "of given files. That means that spider removes any tracked "
    ...
    parser.add_argument(
        ...
        "changed in the Spiderfile.")
    parser.add_argument(
        "--list-input-changes", "--li",
        action="store_true",
        help=
        "List all output files for which the defined input files have changed "
        "in the Spiderfile (e.g. new input files were added in the rule "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "Do not delete the shadow directory on spider startup.")
    parser.add_argument(
        "--allowed-rules",
        nargs="+",
        help=
        "Only consider given rules. If omitted, all rules in Spiderfile are "
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        help=
        "Profile Spider and write the output to FILE. This requires yappi "
        "to be installed.")
    parser.add_argument(
        "--mode",
        ...
        type=int,
        help="Set execution mode of Spider (internal use only)."
    )
    parser.add_argument(
        "--bash-completion",
        action="store_true",
        help="Output code to register bash completion for spider. Put the "
        "following in your .bashrc (including the accents): "
        "`spider --bash-completion` or issue it in an open terminal "
        "session.")
    parser.add_argument(
        "--use-conda",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "to the '.spider' directory relative to the invocation directory. "
    ...
    parser.add_argument(
        "--wrapper-prefix",
        default="https://bitbucket.org/spider/spider-wrappers/raw/",
        help="Prefix for URL created from wrapper directive (default: "
        "https://bitbucket.org/spider/spider-wrappers/raw/). Set this to "
    ...
    parser.add_argument("--no-shared-fs",
                        ...
                        "system. When this flag is activated, Spider will "
...
def main(args = None, argv=None):
    ...
    if args.bash_completion:
        cmd = b"complete -o bashdefault -C spider-bash-completion spider"
    ...
    #
    #     _spidermake = partial(spider, os.path.abspath(args.spiderfile))
    #     gui.register(_spidermake, args)
    #
    #     if ":" in args.gui:
...
def bash_completion(spiderfile="Spiderfile"):
    """Entry point for bash completion."""
    if not len(sys.argv) >= 2:
        print(
            "Calculate bash completion for spider. This tool shall not be invoked by hand.")
        sys.exit(1)
    ...
    if prefix.startswith("-"):
        ...
        elif os.path.exists(spiderfile):
            workflow = Workflow(spiderfile=spiderfile)
            workflow.include(spiderfile)
