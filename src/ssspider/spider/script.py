...
from spider.version import MIN_PY_VERSION
...
def script(path, basedir, input, output, params, wildcards, threads, resources,
    ...
    try:
        with urlopen(sourceurl) as source:
            if path.endswith(".py"):
                spider = Spider(input, output, params, wildcards,
                                      threads, resources, log, config, rulename)
                spider = pickle.dumps(spider)
                # obtain search path for current spider module
                # the module is needed for unpickling in the script
                searchpath = os.path.dirname(os.path.dirname(__file__))
                preamble = textwrap.dedent("""
                ######## Spider header ########
                import sys; sys.path.insert(0, "{}"); import pickle; spider = pickle.loads({}); from spider.logging import logger; logger.printshellcmds = {}
                ######## Original script #########
                """).format(searchpath, spider, logger.printshellcmds)
            ...
            with tempfile.NamedTemporaryFile(
                ...
                if not path.endswith(".Rmd"):
                    ...
                    # Insert Spider object after the RMarkdown header
