...
from spider.io import regex, Namedlist, Wildcards
...
def listfiles(pattern, restriction=None, omit_value=None):
    ...
    Args:
        pattern (str):       a filepattern. Wildcards are specified in spider syntax, e.g. "{id}.txt"
...
def report(text, path,
    ...
    python installation you use with Spider.
    ...
    Args:
        ...
        stylesheet (str):   An optional path to a css file that defines the style of the document. This defaults to <your spider install>/report.css. Use the default to get a hint how to create your own.
    ...
    spider.report.report(text, path,
...
def format(_pattern, *args, stepout=1, _quote_all=False, **kwargs):
    """Format a pattern in Spider style.
...
def read_job_properties(jobscript,
    ...
    """Read the job properties defined in a spider jobscript.

    This function is a helper for writing custom wrappers for the
    spider --cluster functionality. Applying this function to a
...
def min_version(version):
    """Require minimum spider version, raise workflow error if not met."""
    import pkg_resources
    if pkg_resources.parse_version(
        spider.__version__) < pkg_resources.parse_version(version):
        raise WorkflowError(
            "Expecting Spider version {} or higher.".format(version))
