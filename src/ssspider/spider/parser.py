...
from spider.exceptions import WorkflowError
...
class TokenAutomaton:
    ...
    def __init__(self, spiderfile, base_indent=0, dedent=0, root=True):
        self.root = root
...
class Subworkflowspiderfile(SubworkflowKeywordState):
    pass
...
class Subworkflow(GlobalKeywordState):
    ...
    subautomata = dict(spiderfile=Subworkflowspiderfile,
    ...
    def __init__(self, spiderfile, base_indent=0, dedent=0, root=True):
        ...
        self.has_spiderfile = False
        self.has_workdir = False
        self.has_name = False
        self.primary_token = None

    def end(self):
        if not (self.has_spiderfile or self.has_workdir):
            self.error(
                "A subworkflow needs either a path to a Spiderfile or to a workdir.",
                self.primary_token)
        yield ")"
    ...
    def block_content(self, token):
        if is_name(token):
            try:
                if token.string == "spiderfile":
                    self.has_spiderfile = True
...
class Run(RuleKeywordState):
    def __init__(self, spiderfile, rulename,
                 base_indent=0,
                 dedent=0,
                 root=True):
        super().__init__(spiderfile,
    ...
    def start(self):
        ...
        yield ("def __rule_{rulename}(input, output, params, wildcards, threads, "
               "resources, log, version, rule, conda_env, bench_record):".format(
                   rulename=self.rulename if self.rulename is not None else self.spiderfile.rulecount))
...
class AbstractCmd(Run):
    ...
    def __init__(self, spiderfile, rulename,
                 ...
                 dedent=0,
                 root=True):
        super().__init__(spiderfile, rulename,
                         base_indent=base_indent,
                         dedent=dedent,
                         root=root)
        self.cmd = list()
...
class Script(AbstractCmd):
    ...
    def args(self):
        # basedir
        yield ', "{}"'.format(
            os.path.abspath(os.path.dirname(self.spiderfile.path)))
        # other args
...
class Rule(GlobalKeywordState):
    ...
    def name(self, token):
        if is_name(token):
            ...
            self.lineno = self.spiderfile.lines + 1
            self.state = self.block
            for t in self.start():
                yield t, token
        else:
...
def parse(path, overwrite_shellcmd=None, rulecount=0):
    ...
    with Spiderfile(path, rulecount=rulecount) as spiderfile:
        automaton = Python(spiderfile)
        linemap = dict()
        compilation = list()
        for t, orig_token in automaton.consume():
            l = lineno(orig_token)
            linemap.update(dict((i, l) for i in range(
                spiderfile.lines + 1, spiderfile.lines + t.count("\n") + 1)))
            spiderfile.lines += t.count("\n")
            compilation.append(t)
        compilation = "".join(format_tokens(compilation))
        if linemap:
            last = max(linemap)
            linemap[last + 1] = linemap[last]
        return compilation, linemap, spiderfile.rulecount
