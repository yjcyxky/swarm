...
from spider.logging import logger, format_resources, format_resource_names
from spider.rules import Rule, Ruleorder
from spider.exceptions import RuleException, CreateRuleException, \
    UnknownRuleException, NoRulesException, print_exception, WorkflowError
from spider.shell import shell
from spider.dag import DAG
from spider.scheduler import JobScheduler
from spider.parser import parse
import spider.io
from spider.io import protected, temp, temporary, ancient, expand, dynamic, glob_wildcards, flag, not_iterable, touch, unpack, local
from spider.persistence import Persistence
from spider.utils import update_config
from spider.script import script
from spider.wrapper import wrapper
import spider.wrapper
...
class Workflow:
    def __init__(self,
        ...
        self.linemaps = dict()
        self.rule_count = 0
        self.basedir = os.path.dirname(spiderfile)
        self.spiderfile = os.path.abspath(spiderfile)
        self.included = []
        self.included_stack = []
    ...
    def add_rule(self, name=None, lineno=None, spiderfile=None):
        """
        Add a rule.
        """
        if name is None:
        ...
        rule = Rule(name, self, lineno=lineno, spiderfile=spiderfile)
    ...
    def execute(self,
        ...
        if wait_for_files is not None:
            try:
                spider.io.wait_for_files(wait_for_files,
        ...
        try:
            ...
            logger.error(
                ...
                "sure that no other Spider process is trying to create "
                "the same files in the following directory:\n{}\n"
                "If you are sure that no other "
                "instances of spider are running on this directory, "
        ...
        if self.subworkflows and not printdag and not printrulegraph:
            ...
            for subworkflow in self.subworkflows:
                ...
                if subworkflow_targets:
                    ...
                    if not subspidermake(subworkflow.spiderfile,
    ...
    def current_basedir(self):
        """Basedir of currently parsed Spiderfile."""
        assert self.included_stack
        return os.path.abspath(os.path.dirname(self.included_stack[-1]))

    def include(self, spiderfile,
                overwrite_first_rule=False,
                print_compilation=False,
                overwrite_shellcmd=None):
        """
        Include a spiderfile.
        """
        # check if spiderfile is a path to the filesystem
        if not urllib.parse.urlparse(spiderfile).scheme:
            if not os.path.isabs(spiderfile) and self.included_stack:
                spiderfile = os.path.join(self.current_basedir, spiderfile)
        ...
        if spiderfile in self.included:
            logger.info("Multiple include of {} ignored".format(spiderfile))
            return
        self.included.append(spiderfile)
        self.included_stack.append(spiderfile)
        ...
        code, linemap, rulecount = parse(spiderfile,
        ...
        sys.path.insert(0, os.path.dirname(spiderfile))

        self.linemaps[spiderfile] = linemap
        exec(compile(code, spiderfile, "exec"), self.globals)
    ...
    def configfile(self, jsonpath):
        ...
        c = spider.io.load_configfile(jsonpath)
    ...
    def subworkflow(self, name, spiderfile=None, workdir=None, configfile=None):
        ...
        sw = Subworkflow(self, name, spiderfile, workdir, configfile)
    ...
    def rule(self, name=None, lineno=None, spiderfile=None):
        name = self.add_rule(name, lineno, spiderfile)
        ...
        def decorate(ruleinfo):
            ...
            if ruleinfo.wrapper:
                rule.conda_env = spider.wrapper.get_conda_env(
...
class Subworkflow:
    def __init__(self, workflow, name, spiderfile, workdir, configfile):
        self.workflow = workflow
        self.name = name
        self._spiderfile = spiderfile
        self._workdir = workdir
        self.configfile = configfile

    @property
    def spiderfile(self):
        if self._spiderfile is None:
            return os.path.abspath(os.path.join(self.workdir, "Spiderfile"))
        if not os.path.isabs(self._spiderfile):
            return os.path.abspath(os.path.join(self.workflow.basedir,
                                                self._spiderfile))
        return self._spiderfile
...
def srcdir(path):
    """Return the absolute path, relative to the source directory of the current Spiderfile."""
