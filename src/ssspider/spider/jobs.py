...
from spider.io import IOFile, Wildcards, Resources, _IOFile, is_flagged, contains_wildcard, lstat
from spider.utils import format, listfiles
from spider.exceptions import RuleException, ProtectedOutputException, WorkflowError
from spider.exceptions import UnexpectedOutputException, CreateCondaEnvironmentException
from spider.logging import logger
from spider.common import DYNAMIC_FILL, lazy_property
from spider import conda, wrapper
...
class Job:
    ...
    def prepare(self):
        ...
        if self.rule.shadow_depth == "shallow":
            ...
            spidermake_dir = os.path.join(cwd, ".spider")
            for dirpath, dirnames, filenames in os.walk(cwd):
                # Must exclude .spider and its children to avoid infinite
                # loop of symlinks.
                if os.path.commonprefix([spidermake_dir, dirpath
                                         ]) == spidermake_dir:
                    continue
