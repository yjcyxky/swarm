...
from spider.jobs import jobfiles
from spider.utils import listfiles


class Persistence:
    def __init__(self, nolock=False, dag=None, conda_prefix=None, warn_only=False):
        self.path = os.path.abspath(".spider")
    ...
    def lock_warn_only(self):
        if self.locked:
            logger.info(
                ...
                "means that another Spider instance is running on this directory."
                "Another possiblity is that a previous run exited unexpectedly.")

    def lock(self):
        if self.locked:
            raise IOError("Another spider process "
