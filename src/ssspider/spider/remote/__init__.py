...
class AbstractRemoteProvider:
    ...
    def remote(self, value, *args, keep_local=None, stay_on_remote=None, static=False, **kwargs):
        if spider.io.is_flagged(value, "temp"):
            raise SyntaxError(
                "Remote and temporary flags are mutually exclusive.")
        if spider.io.is_flagged(value, "protected"):
            raise SyntaxError(
        ...
        return spider.io.flag(value, "remote_object", remote_object)
    ...
    def glob_wildcards(self, pattern, *args, **kwargs):
        ...
        referenceObj = spider.io.IOFile(self.remote(pattern, *args, **kwargs))
        ...
        return spider.io.glob_wildcards(pattern, files=key_list)
