...
ssansible.opt = {}
ssansible.opt.holder = undefined
ssansible.opt.host = "//"+window.location.host

ssansible.model = {}

ssansible.model.nowTime = 0;
    
ssansible.start = function(options)
{
    for(var i in options)
    {
        if(ssansible.opt[i] && typeof(ssansible.opt[i]) == "object")
        {
            for(var j in options[i])
            {
                ssansible.opt[i][j] = options[i][j]
            }
        }
        ssansible.opt[i] = options[i]
    }

    spajs.init({
        holder: ssansible.opt.holder,
        menu_url: undefined,
        useHistoryApi:true
    })
    ...
    {
        var t = new Date();
        ssansible.model.nowTime = t.getTime();
    }, 5001)
    ...
    tabSignal.emit("ssansible.start")

    spajs.openMenuFromUrl()
}
 
ssansible.showErrors = function(res)
{
    if(!res)
    {
        return true;
    ...
    for(var i in res)
    {
        ...
        {
            return ssansible.showErrors(res[i])
        }
    }
}
