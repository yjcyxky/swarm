				...
				regExpEscape = function (str) {
					return String(str).replace(escapeExp, '\\$1');
                                ...
                                reactiveReplace = function(html)
                                {
                                    /**
                                     * Блок отслеживания
                                     * В конструкцию включается <~ ssansible.model.groups[item_id].groups > переменная которую надо отслеживать
                                     * При её изменении просто перерисовывается вложеный в блок кусочек шаблона
                                     * 
                                     * 
                                        <~ ssansible.model.groups[item_id].groups > 
                                            <% for(var i in ssansible.model.groups[item_id].groups){ %>
                                                <div class="form-group col-lg-2">
                                                    <label class="control-label" >Group</label>
                                                    <%- ssansible.model.groups[item_id].groups[i].name %>
                                                </div>
                                            <% } %>
                                        <~>  
                                     * 
                                    ...
                                    for(var i in parts)
                                    {
                                        ...
                                        if(val.charAt(0) == '~' && val.charAt(1) == '>')
                                        {
                                            ...
                                            if(status != 0 && status == lastStatus)
                                            {
                                                ...
                                                { 
                                                    // Запись вида <~ ssansible.model.groups[item_id] >
                                                    forObject = forObject.match(/^(.*)\[([^\]]+)\]$/) 
                                                }
                                                else if(forObject.indexOf(".") != -1)
                                                {
                                                    // Запись вида <~ ssansible.model.groups[item_id].groups > 
                                                    forObject = forObject.match(/^(.*)\.([^.]+)$/)
                                                    forObject[2] = "'"+forObject[2]+"'"
                                                }
                                                else
                                                {
                                                    // Запись вида <~ ssansible >
                                                    // var j in line
                                                    forObject = []
                                                ...
                                                // Для отслеживания элемента массива по индексу  <~ ssansible.model.groups[item_id] > надо писать без одинарных кавычек forObject[2]
